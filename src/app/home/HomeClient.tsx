...
export default function HomeClient({
  profile, totalCommitments, checks, email,
}: { profile: Profile; totalCommitments: number; checks: Check[]; email: string }) {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-[420px] px-5 pb-28 pt-6">
      <div className="mb-6 flex items-center justify-between">
        <span className="font-display text-[16px] font-semibold text-forest">Check<span className="text-mint">.</span>it</span>
        <button onClick={signOut} title="Sign out"
          className="flex h-8 w-8 items-center justify-center rounded-full bg-forest text-[12px] font-semibold text-mint">
          {email ? email[0].toUpperCase() : 'U'}
        </button>
      </div>

      <p className="mb-2.5 text-[9px] font-medium uppercase tracking-[0.12em] text-placeholder">Your financial snapshot</p>
      <div className="rounded-2xl border-[0.5px] border-border bg-white p-4">
        <div className="grid grid-cols-2 gap-2">
          <div className="rounded-xl bg-sage-whisper p-3.5">
            <p className="text-[10px] text-sage">Free cash / month</p>
            <p className={`font-display text-[20px] font-semibold ${freeCash < 0 ? 'text-warn' : 'text-forest'}`}>₹{mask(inr(freeCash))}</p>
          </div>
          <div className="rounded-xl bg-forest p-3.5">
            <p className="text-[10px] text-mint">Emergency buffer</p>
            <p className="font-display text-[20px] font-semibold text-[#E8F5EE]">₹{mask(inr(profile.liquid_buffer))}</p>
          </div>
        </div>
        ...
        <div className="mt-3.5 flex items-center justify-between border-t border-divider pt-3">
          ...
          <button onClick={() => router.push('/onboarding')} className="text-[12px] font-medium text-mint">Modify financials →</button>
        </div>
      </div>

      <p className="mb-2.5 mt-7 text-[9px] font-medium uppercase tracking-[0.12em] text-placeholder">Recent checks</p>
      {checks.length === 0 ? (
              ...
              <div key={c.id} className="flex items-center rounded-xl border-[0.5px] border-border bg-white px-4 py-3">
                <div>
                  ...
                  <p className="text-[10px] text-sage">
                    {c.purchase_type === 'emi' ? 'EMI' : 'One-time'} · {new Date(c.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}
                  </p>
                </div>
                <div className="ml-auto flex items-center gap-2.5">
                  ...
                  <span className={`flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-medium ${p.bg} ${p.text}`}>
                    <span className="h-1.5 w-1.5 rounded-full" style={{ background: p.dot }} />{p.label}
                  </span>
                </div>
              </div>
            )
      ...
      <div className="fixed inset-x-0 bottom-0 mx-auto max-w-[420px] px-5 pb-5 pt-8"
        style={{ background: 'linear-gradient(to top, var(--color-sage-whisper) 55%, transparent)' }}>
...
function Row({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <span className="font-number font-medium text-forest">
        {value}{sub && <span className="ml-1 text-[10px] font-normal text-placeholder">({sub})</span>}
      </span>
    </div>
  )
}